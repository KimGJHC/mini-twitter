...
class CommentApiTests(TestCase):
    ...
    def test_comments_count_with_cache(self):
        ...
        comment_data = self.david_client.post(COMMENT_URL, data).data
        ...
        response = self.david_client.put(comment_url, {'content': 'updated'})
        self.assertEqual(response.status_code, 200)
        response = self.david_client.get(tweet_url)
        self.assertEqual(response.data['comments_count'], 3)
        self.tweet.refresh_from_db()
        ...
        response = self.david_client.delete(comment_url)
        self.assertEqual(response.status_code, 200)
        response = self.kim_client.get(tweet_url)
