...
class LikeApiTests(TestCase):
    ...
    def test_likes_count(self):
        ...
        response = self.kim_client.get(tweet_url)
        self.assertEqual(response.data['likes_count'], 1)
        tweet.refresh_from_db()
        self.assertEqual(tweet.likes_count, 1)

        # david canceled likes
        self.kim_client.post(LIKE_BASE_URL + 'cancel/', data)
        tweet.refresh_from_db()
        self.assertEqual(tweet.likes_count, 0)
        response = self.david_client.get(tweet_url)
        self.assertEqual(response.data['likes_count'], 0)

    def test_likes_count_with_cache(self):
        tweet = self.create_tweet(self.kim)
        self.create_newsfeed(self.kim, tweet)
        self.create_newsfeed(self.david, tweet)
        ...
        response = self.kim_client.get(newsfeed_url)
        self.assertEqual(response.data['results'][0]['tweet']['likes_count'], 4)
        response = self.david_client.get(newsfeed_url)
        self.assertEqual(response.data['results'][0]['tweet']['likes_count'], 4)

        # david canceled likes
        self.david_client.post(LIKE_BASE_URL + 'cancel/', data)
        tweet.refresh_from_db()
